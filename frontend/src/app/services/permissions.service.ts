...
export class PermissionsService {
  ...
  canCreateEmployee(user: User | null): boolean {
    return this.isAdmin(user) || this.isDepartmentHeadRH(user);
  }
  ...
  canUpdateEmployee(currentUser: User | null, targetUser?: User): boolean {
    ...
    if(this.isDepartmentHeadRH(currentUser)) {
      if(targetUser?.role === 'ADMINISTRATEUR' && targetUser !=null) {
        return false;
      }
      return !isSelf;
    }
    ...
    if (this.isDepartmentHead(currentUser) && !this.isDepartmentHeadRH(currentUser)) {
  ...
  canDeleteEmployee(currentUser: User | null, targetUser?: User): boolean {
    ...
    if (!this.isAdmin(currentUser) && !this.isDepartmentHeadRH(currentUser)) return false;
  ...
  canUpdateSalary(currentUser: User | null, targetUser?: User): boolean {
    ...
    if (this.isAdmin(currentUser) || this.isDepartmentHeadRH(currentUser)) return true;
